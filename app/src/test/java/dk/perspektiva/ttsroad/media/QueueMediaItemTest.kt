...
class QueueMediaItemTest {
    ...
    private val serverUrl = "https://server.example.com"

    private fun item(
        entry: QueueItem = this.entry,
        host: String? = serverUrl,
    ) = TtsRoadMediaItems.queueItem(entry, host)
    ...
    fun `the extras progress sync reads are all present`() {
        val extras = item(entry)?.mediaMetadata?.extras
    ...
    fun `an entry with no audio is not offered`() {
        ...
        assertNull(item(entry.copy(audio = null)))
    }

    @Test
    fun `the fiction is the subtitle, because the list crosses books`() {
        val metadata = item(entry)?.mediaMetadata
    ...
    fun `a duration is carried in milliseconds`() {
        assertEquals(1_800_000L, item(entry)?.mediaMetadata?.durationMs)
    }

    @Test
    fun `an unstarted chapter carries no resume position`() {
        val extras = item(entry.copy(positionSeconds = 0.0))
            ?.mediaMetadata
            ?.extras
    ...
    fun `the audio host is rewritten to the address actually signed in to`() {
        ...
        val item = item(entry, host = "http://10.0.2.2:8000")
    ...
    fun `an untitled entry still has something to show`() {
        val metadata = item(entry.copy(chapterTitle = null))?.mediaMetadata

        assertEquals("Chapter", metadata?.title)
    }

    @Test
    fun `an entry with an unusable serverUrl is not offered`() {
        assertNull(item(entry, host = null))
        assertNull(item(entry, host = ""))
        assertNull(item(entry, host = "not-a-url"))
    }
}
